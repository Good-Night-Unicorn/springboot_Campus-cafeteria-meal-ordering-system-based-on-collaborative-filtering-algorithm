<template>
	...
	<div class="container" :style='{"minHeight":"100vh","alignItems":"center","background":"url(http://codegen.caihongy.cn/20231014/825464b6febd4bf2946d4b4b656195e3.jpg)","display":"flex","width":"100%","backgroundSize":"cover","backgroundPosition":"center center","backgroundRepeat":"no-repeat","justifyContent":"center"}'>
		<el-form class='rgs-form' v-if="pageFlag=='register'" :style='{"padding":"50px 0 20px","boxShadow":"0px 26px 26px -30px #999","margin":"120px auto 60px","borderRadius":"12px","background":"radial-gradient(circle, rgba(245,245,245,1) 0%, rgba(255,255,255,1) 50%, rgba(245,245,245,1) 100%)","width":"35vw","position":"relative","height":"auto"}' ref="registerForm" :model="registerForm" :rules="rules">
			...
			<div v-if="true" :style='{"margin":"20px auto 10px","color":"#333","top":"-90px","textAlign":"center","width":"100%","letterSpacing":"4px","lineHeight":"44px","fontSize":"30px","position":"absolute"}'>基于协同过滤算法的校园食堂订餐系统注册</p></div>
			...
			<el-form-item :style='{"width":"80%","padding":"0","margin":"0 auto 15px","height":"auto"}' v-if="tableName=='shangjia'" prop="dianpuzhanghao">
				<div v-if="true" :style='{"padding":"0 10px","color":"#333","textAlign":"right","background":"none","display":"inline-block","width":"90px","lineHeight":"36px","fontSize":"14px"}' :class="changeRules('dianpuzhanghao')?'required':''">店铺账号：</div>
				<el-input v-model="registerForm.dianpuzhanghao"  placeholder="请输入店铺账号" />
			</el-form-item>
			<el-form-item :style='{"width":"80%","padding":"0","margin":"0 auto 15px","height":"auto"}' v-if="tableName=='shangjia'" prop="dianpumingcheng">
				<div v-if="true" :style='{"padding":"0 10px","color":"#333","textAlign":"right","background":"none","display":"inline-block","width":"90px","lineHeight":"36px","fontSize":"14px"}' :class="changeRules('dianpumingcheng')?'required':''">店铺名称：</div>
			...
			<el-form-item :style='{"width":"80%","padding":"0","margin":"0 auto 15px","height":"auto"}' v-if="tableName=='shangjia'" prop="dianpujianjie">
				<div v-if="true" :style='{"padding":"0 10px","color":"#333","textAlign":"right","background":"none","display":"inline-block","width":"90px","lineHeight":"36px","fontSize":"14px"}' :class="changeRules('dianpujianjie')?'required':''">店铺简介：</div>
				<el-input v-model="registerForm.dianpujianjie"  placeholder="请输入店铺简介" />
			</el-form-item>
			<el-form-item :style='{"width":"80%","padding":"0","margin":"0 auto 15px","height":"auto"}' v-if="tableName=='shangjia'" prop="dianpudizhi">
				<div v-if="true" :style='{"padding":"0 10px","color":"#333","textAlign":"right","background":"none","display":"inline-block","width":"90px","lineHeight":"36px","fontSize":"14px"}' :class="changeRules('dianpudizhi')?'required':''">店铺地址：</div>
				<el-input v-model="registerForm.dianpudizhi"  placeholder="请输入店铺地址" />
			</el-form-item>
			<el-form-item :style='{"width":"80%","padding":"0","margin":"0 auto 15px","height":"auto"}' v-if="tableName=='shangjia'" prop="lianxifangshi">
			...
			<el-form-item :style='{"width":"80%","padding":"0","margin":"0 auto 15px","height":"auto"}' v-if="tableName=='shangjia'" prop="dianpuzhaopian">
				<div v-if="true" :style='{"padding":"0 10px","color":"#333","textAlign":"right","background":"none","display":"inline-block","width":"90px","lineHeight":"36px","fontSize":"14px"}' :class="changeRules('dianpuzhaopian')?'required':''">店铺照片：</div>
                <file-upload
					tip="点击上传店铺照片"
					action="file/upload"
					:limit="1"
...
export default {
	...
	mounted() {
		if(this.$route.query.pageFlag=='register'){
			...
			if ('shangjia' == this.tableName) {
				this.requiredRules.dianpuzhanghao = [{ required: true, message: '请输入店铺账号', trigger: 'blur' }]
			}
			if ('shangjia' == this.tableName) {
    ...
    created() {
		...
		if(this.$route.query.pageFlag=='register'){
		  ...
		  if ('shangjia' == this.tableName) {
			this.rules.dianpuzhanghao = [{ required: true, message: '请输入店铺账号', trigger: 'blur' }];
		  }
		  if ('shangjia' == this.tableName) {
          ...
          if (valid) {
            var url=this.tableName+"/register";
               ...
               if(`yonghu` == this.tableName && this.registerForm.mima!=this.registerForm.mima2) {
				...
				if((!this.registerForm.dianpuzhanghao) && `shangjia` == this.tableName){
               ...
               if(`shangjia` == this.tableName && this.registerForm.mima!=this.registerForm.mima2) {
				...
				if((!this.registerForm.mima) && `shangjia` == this.tableName){
					...
					if(`shangjia` == this.tableName && this.registerForm.lianxifangshi &&(!this.$validate.isMobile2(this.registerForm.lianxifangshi))){
